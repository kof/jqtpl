 ...
 *
 * Originally started asa port of jQuery's Template Engine to Nodejs.
...
function compile(markup) {
    ...
    return new Function('__jqtpl', '__data',
        'var __res=[];' +

        // Introduce the data as local variables using with(){}
        'with(__data){__res.push("' +
...
exports.tag = {
    partial: {
        ...
        open: 'if(__notnull_1){__res=__res.concat(__jqtpl.partial(__1,__2,__data));}'
    },
    each: {
        _default: { __2: '$value, $index' },
        open: 'if(__notnull_1){this.each(__1a,function(__2){with(this){',
        close: '}});}'
    },
    verbatim: {},
    html: {
        // Unecoded expression evaluation.
        open: 'if(__notnull_1){__res.push(__1a);}'
    },
    'if': {
    ...
    '=': {
        _default: { __1: '__data' },
        open: 'if(__notnull_1){__res.push(__jqtpl.escapeHtml(__1a));}'
    },
...
exports.compile = function(markup, name) {
    ...
    name || (name = markup);
    fn = exports.cache[name];

    if (!fn) {
        // Generate a reusable function that will serve as a template
        // generator (and which will be cached).
        try {
            fn = exports.cache[name] = compile(markup);
        } catch(err) {
            err.message += ': ' + markup;
            throw err;
        }
 ...
 * @api public
 */
exports.render = exports.partial = function(markup, data) {
    var fn = exports.compile(markup),
        ret, i;

    if (data && isArray(data)) {
        ret = '';
        for (i = 0; i < data.length; ++i) {
            ret += fn(exports, data[i]);
        }
    } else {
        ret = fn(exports, data || {});
    }
